...
void onsetsds_init(OnsetsDS* ods, float* odsdata, int fftformat, int odftype, size_t fftsize, unsigned int medspan,
    ...
    // Also point the other pointers to the right places (in Max, these translate to array index values where odsdata = 0, I think...)
    ods->curr = (OdsPolarBuf*)odsdata; // assigns the pointer address of odsdata (a float*) to ods->curr (an OdsPolarBuf*). This casting allows the function to interpret the data at odsdata as an OdsPolarBuf structure. This is valid as long as the memory pointed to by odsdata is organized in a way that matches the OdsPolarBuf structure's layout [dc, nyq, bin1[0](mag), bin1[1](phase), bin2[0](mag), bin2[1](phase), etc...]
...
//processes just 1 fft frame at a time AFAICT...
...
//FFT format switch in this function, but ODS_FFT_SC3_POLAR only ever used (as seen in Onsets_next() in Onsets.cpp where onsetsds_init() configured to use this FFT value only...
...
//big ODS (onset detection mode) switch in here...
