...
typedef struct OdsPolarBuf {
    float dc, nyq;
    OdsPolarBin bin[1]; //An array of OdsPolarBin structures with a size of 1. Although the OdsPolarBuf structure is defined with a fixed-size array of one OdsPolarBin, you can allocate additional memory to accommodate more OdsPolarBin elements at runtime. This approach allows you to use a dynamically sized array within the structure, even though it is statically declared with a size of 1.
} OdsPolarBuf;
